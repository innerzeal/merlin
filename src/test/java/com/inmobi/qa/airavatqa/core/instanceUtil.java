...
import com.inmobi.qa.airavatqa.core.ProcessInstancesResult.InstanceAction;
import com.inmobi.qa.airavatqa.core.ProcessInstancesResult.ProcessInstance;
...
import com.inmobi.qa.airavatqa.generated.feed.Locations;
...
public class instanceUtil {
	...
	public static ProcessInstancesResult hitUrl(String url,HttpRequestBase request) throws Exception {
		...
		else if(jsonString.contains("Parameter start is empty") || jsonString.contains("Unparseable date:"))
		{
			...
			return r;
				...
				||(response.getStatusLine().getStatusCode()==400 && jsonString.contains("is after process end"))||(response.getStatusLine().getStatusCode()==400 && jsonString.contains("is before PROCESS's  start")))
		{
			r.setStatusCode(400);
			return r;
		}
	...
	public static void  validateSuccess(ProcessInstancesResult r,Bundle b,WorkflowStatus ws) throws Exception
	{
	...
	public static int runningInstancesInResult(ProcessInstancesResult r,WorkflowStatus ws) {
	...
	public static void validateSuccessOnlyStart(ProcessInstancesResult r,
			Bundle b, WorkflowStatus ws) throws Exception {
		Assert.assertEquals(r.getStatus(),APIResult.Status.SUCCEEDED);
	...
	public static void validateResponse(ProcessInstancesResult r, int totalInstances, int runningInstances,
		...
		for(int instanceIndex = 0 ; instanceIndex < pArray.length; instanceIndex++)
		{
			...
			if(pArray[instanceIndex].getStatus().equals(WorkflowStatus.RUNNING))
				actualRunningInstances++;
			else if(pArray[instanceIndex].getStatus().equals(WorkflowStatus.SUSPENDED))
				actualSuspendedInstances++;
			else if(pArray[instanceIndex].getStatus().equals(WorkflowStatus.WAITING))
				actualWaitingInstances++;
			else if(pArray[instanceIndex].getStatus().equals(WorkflowStatus.KILLED))
	...
	public static String getTimeWrtSystemTime(int minutes) throws Exception {
		...
		Util.print("SystemDate after offset: "+time.getTime() + " toString: "+time.getTime().toString()+ "toGMtString: "+time.getTime().toGMTString());
	...
	public static String getCoordWorkFlowCoordinator(String defaultCoordId) throws Exception {
		...
		conf.set("fs.default.name","hdfs://"+Util.readPropertiesFile("hadoop_url"));
	...
	public static COORDINATORAPP getLateCoordinatorApp(
			String entityName,int coordNumber) throws Exception {
		String coordStr = getCoordWorkFlowCoordinator(getLateCoordIDFromProcess(entityName,coordNumber));
		JAXBContext jc=JAXBContext.newInstance(COORDINATORAPP.class);
		Unmarshaller u=jc.createUnmarshaller();
	...
	public static void putDataInFolders(
		...
		for(int i = 0 ; i < inputFoldersForInstance.size() ; i++)
			putDataInFolder(inputFoldersForInstance.get(i),type);

	}


	public static void putDataInFolder(final String remoteLocation) throws Exception {
	...
	public static void putDataInFolder(final String remoteLocation,String type) throws Exception {
	...
	public static void putFileInFolders(ArrayList<String> folderList,
			final String fileName) throws Exception {
		Configuration conf=new Configuration();
		conf.set("fs.default.name",Util.readPropertiesFile("cluster_write"));
		...
		for(final String folder:folderList)
		{
			...
			user.doAs(new PrivilegedExceptionAction<Boolean>() {
				...
				@Override
				public Boolean run() throws Exception {
					logger.info("copying  "+fileName+" to "+folder);
					fs.copyFromLocalFile(new Path(fileName),new Path(folder));
					return true;
	...
	public static ArrayList<String> getMissingDependencyForInstance(
	...
	public static void verifyStatusLog(ProcessInstancesResult r) throws Exception {
		...
		for(ProcessInstance instance:r.getInstances()){


			if(instance.getStatus().equals(WorkflowStatus.WAITING)){

				Assert.assertEquals(instance.getLogFile(),"-");
				break;
			}
			else if(instance.getStatus().equals(WorkflowStatus.RUNNING) || instance.getStatus().equals(WorkflowStatus.SUSPENDED) || instance.getStatus().equals(WorkflowStatus.KILLED))
			{
			...
			for( InstanceAction action:instance.actions){
	...
	public static Cluster getClusterElement(Bundle bundle) throws JAXBException {
		JAXBContext jc=JAXBContext.newInstance(Cluster.class); 
		Unmarshaller u=jc.createUnmarshaller();
		Cluster clusterElement=(Cluster)u.unmarshal((new StringReader(bundle.getClusters().get(0))));

		return clusterElement;
	}

	public static void writeClusterElement(Bundle bundle,Cluster c) throws JAXBException {
		JAXBContext jc=JAXBContext.newInstance(Cluster.class); 
		java.io.StringWriter sw = new StringWriter();
		Marshaller marshaller = jc.createMarshaller();
	...
	public static void verifyDataInTarget(IEntityManagerHelper helper,
		...
		List<com.inmobi.qa.airavatqa.generated.feed.Cluster> sourceClusters = new ArrayList<com.inmobi.qa.airavatqa.generated.feed.Cluster>();
		List<com.inmobi.qa.airavatqa.generated.feed.Cluster> targetClusters = new ArrayList<com.inmobi.qa.airavatqa.generated.feed.Cluster>();


		List<com.inmobi.qa.airavatqa.generated.feed.Cluster> clusterList = f.getClusters().getCluster();
	...
	public static String setProcessCluster(String process,
			String clusterName, com.inmobi.qa.airavatqa.generated.process.Validity validity) throws Exception {


		com.inmobi.qa.airavatqa.generated.process.Cluster c = new com.inmobi.qa.airavatqa.generated.process.Cluster();
	...
	static Process getProcessElement(String process) throws JAXBException {
		JAXBContext jc=JAXBContext.newInstance(Process.class); 
		Unmarshaller u=jc.createUnmarshaller();
	...
	public static ArrayList<String> getInputFoldersForInstanceForReplication(
		...
		return instanceUtil.getReplicationFolderFromInstanceRunConf(coordInfo.getActions().get(instanceNumber).getRunConf());
	}
	...
	public static String ClusterElementToString(Cluster c) throws JAXBException {
		JAXBContext jc=JAXBContext.newInstance(Cluster.class); 
		java.io.StringWriter sw = new StringWriter();
		Marshaller marshaller = jc.createMarshaller();
		marshaller.marshal(c,sw);
	...
	private static String getClusterName(String clusterData) throws Exception {
		...
		Cluster c = instanceUtil.getClusterElement(clusterData);
		return c.getName();		
	}

	static Cluster getClusterElement(String clusterData) throws Exception{
		JAXBContext jc=JAXBContext.newInstance(Cluster.class); 
		Unmarshaller u=jc.createUnmarshaller();
		Cluster clusterElement=(Cluster)u.unmarshal((new StringReader(clusterData)));
